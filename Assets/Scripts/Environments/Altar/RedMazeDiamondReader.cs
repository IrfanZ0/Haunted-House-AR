...
public class RedMazeDiamondReader : MonoBehaviour, IBlueDiamondReader, IOrangeDiamondReader, IPurpleDiamondReader, IYellowDiamondReader
{
    ...
    private void OnTriggerEnter ( Collider other )
    {
        ...
        switch ( diamond.name )
        {
            ...
            case "Purple Diamond":
                {
                    if ( PurpleDiamondScanning ( diamond ) )
                    {
                        PurpleDiamondTriggering ( diamond );
                    }
                    else
                    {
                        PurpleDiamondDestroying ( diamond );
                    }
    ...
    {
        StartCoroutine ( AltarSpeaks ( purpleDiamond.name ) );
        StartCoroutine ( RevealPassage ( altar ) );
    }
    ...
    private IEnumerator AltarSpeaks ( string diamondName )
    {
        ...
        yield return new WaitForSeconds ( 5f );
        altarTextBox.text = "From here you could be transported to Orange, Purple, Blue, and Yellow mazes.";
        yield return new WaitForSeconds ( 5f );
