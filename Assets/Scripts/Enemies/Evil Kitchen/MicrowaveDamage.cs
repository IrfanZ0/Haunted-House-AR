...
public class MicrowaveDamage : MonoBehaviour
{

    Slider lifeSlider;
    Animator microwaveAnim;
    ...
    void Start()
    {
        ...
        microwaveAnim = gameObject.GetComponent<Animator>();
    ...
    {
        lifeSlider.value -= 3f;
        microwaveAnim.SetTrigger("Damage");

        if (lifeSlider.value < 1f)
        {
            microwaveAnim.SetBool("isDead", true);
